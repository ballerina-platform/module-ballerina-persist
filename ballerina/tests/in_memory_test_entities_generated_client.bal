...
public isolated client class InMemoryTestEntitiesClient {
    *AbstractPersistClient;
    ...
    public isolated function init() returns Error? {
        ...
        self.persistClients[ALL_TYPES] = check new (metadata.get(ALL_TYPES).cloneReadOnly());
        self.persistClients[STRING_ID_RECORD] = check new (metadata.get(STRING_ID_RECORD).cloneReadOnly());
        self.persistClients[INT_ID_RECORD] = check new (metadata.get(INT_ID_RECORD).cloneReadOnly());
        self.persistClients[FLOAT_ID_RECORD] = check new (metadata.get(FLOAT_ID_RECORD).cloneReadOnly());
        self.persistClients[DECIMAL_ID_RECORD] = check new (metadata.get(DECIMAL_ID_RECORD).cloneReadOnly());
        self.persistClients[BOOLEAN_ID_RECORD] = check new (metadata.get(BOOLEAN_ID_RECORD).cloneReadOnly());
        self.persistClients[COMPOSITE_ASSOCIATION_RECORD] = check new (metadata.get(COMPOSITE_ASSOCIATION_RECORD).cloneReadOnly());
        self.persistClients[ALL_TYPES_ID_RECORD] = check new (metadata.get(ALL_TYPES_ID_RECORD).cloneReadOnly());
    };
