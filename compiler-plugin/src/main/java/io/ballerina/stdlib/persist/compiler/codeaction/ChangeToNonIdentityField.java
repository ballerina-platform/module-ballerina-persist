...
import static io.ballerina.stdlib.persist.compiler.codeaction.PersistCodeActionName.CHANGE_TO_NON_IDENTITY_FIELD;

/**
 * Change to non-identity field code action.
 */
public class ChangeToNonIdentityField extends AbstractRemoveUnsupportedSyntax {
    @Override
    protected String getName() {
        return CHANGE_TO_NON_IDENTITY_FIELD.getName();
    }

    @Override
    protected List<String> getSupportedDiagnosticCodes() {
        return List.of(PERSIST_503.getCode());
    }
    ...
    protected TextRange getNodeLocation(Diagnostic diagnostic) {
        ...
        return TextRange.from(startOffset, 9);
    }
}
